...
public class BaseCountdown {
    ...
    public boolean isShowDay, isShowHour, isShowMinute, isShowSecond, isShowMillisecond, isShowMilliseconInSecond, isHideSuffixSecond;
    //    private boolean isShowMilliseconInSecond = false;
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        isHideSuffixSecond = ta.getBoolean(R.styleable.CountdownView_isHideSuffixSecond, false);
        isShowMillisecond = ta.getBoolean(R.styleable.CountdownView_isShowMillisecond, false);
        if (isShowMillisecond) {
            isShowMilliseconInSecond = ta.getBoolean(R.styleable.CountdownView_isShowMillisecondInSecond, false);
        }
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    protected final float getAllContentWidthBase(float timeWidth) {
        ...
        if (isConvertDaysToHours) {
            width += getDayAndHourContentWidth() + Utils.dp2px(mContext, 10);
        } else if (isShowHour) {
            width += timeWidth;
        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowSecond) {
            if (isShowMilliseconInSecond) {
                canvas.drawText(Utils.formatNum(mSecond) + "." + Utils.formatMillisecond(mMillisecond), Utils.dp2px(mContext, 5) + mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            } else {
                // draw second text
            ...
            if (mSuffixSecondTextWidth > 0) {
                if (isShowMilliseconInSecond) {
                    if (!isHideSuffixSecond) {//毫秒以10 为单位,走隐藏"秒"或者":"逻辑
                        canvas.drawText(mSuffixSecond, Utils.dp2px(mContext, 10) + mSecondLeft + mTimeTextWidth + mSuffixSecondLeftMargin, mSuffixSecondTextBaseline, mSuffixTextPaint);
                    }
                } else {
                    // draw second suffix
