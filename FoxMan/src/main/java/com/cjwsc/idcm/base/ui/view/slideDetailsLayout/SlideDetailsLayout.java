package com.cjwsc.idcm.base.ui.view.slideDetailsLayout;
...
//                top = t + offset;
        ...
        switch (aciton) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (canChildScrollVertically((int) yDiff)) {
                    shouldIntercept = false;
                    if (SlideDebug.DEBUG) {
                        LogUtil.d(SlideDebug.TAG, "intercept, child can scroll vertically, do not intercept");
                    }
                } else {
                    ...
                    if (yDiffabs > mTouchSlop && yDiffabs >= xDiffabs
                        ...
                        if (SlideDebug.DEBUG) {
                            LogUtil.d(SlideDebug.TAG, "intercept, intercept events");
                        }
                    }
                }
                break;
    ...
    private void processTouchEvent(final float offset) {
        ...
        if (SlideDebug.DEBUG) {
            LogUtil.v("slide", "process, offset: " + mSlideOffset);
        }
        // relayout
        requestLayout();
    }
    ...
    private void finishTouchEvent() {
        ...
        if (SlideDebug.DEBUG) {
            LogUtil.v(SlideDebug.TAG, "finish, offset: " + offset + ", percent: " + percent + ", yVelocity: " + yVelocity);
        }
