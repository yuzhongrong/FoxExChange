package com.cjwsc.idcm.base.ui.view.countdownview;
...
public class BackgroundCountdown extends BaseCountdown {
    ...
    private void initTimeBgRect(float topPaddingSize) {
        ...
        if (isShowSecond) {
            ...
            if (isShowTimeBgBorder) {
                float rectBgBorderRight = 0;
                float rectBgRight = 0;
                if (isShowMilliseconInSecond) {
                    rectBgBorderRight = mSecondLeft + mTimeBgSize + (mTimeBgBorderSize * 2) + Utils.dp2px(mContext, 10);
                    rectBgRight = mSecondLeft + mTimeBgSize + mTimeBgBorderSize + Utils.dp2px(mContext, 10);
                } else {
                    rectBgBorderRight = mSecondLeft + mTimeBgSize + (mTimeBgBorderSize * 2);
                    rectBgRight = mSecondLeft + mTimeBgSize + mTimeBgBorderSize;
                }
                mSecondBgBorderRectF = new RectF(mSecondLeft, topPaddingSize, rectBgBorderRight, topPaddingSize + mTimeBgSize + (mTimeBgBorderSize * 2));
                mSecondBgRectF = new RectF(mSecondLeft + mTimeBgBorderSize, topPaddingSize + mTimeBgBorderSize, rectBgRight, topPaddingSize + mTimeBgSize + mTimeBgBorderSize);
            } else {
                float rectBgRight1 = 0;
                if (isShowMilliseconInSecond) {
                    rectBgRight1 = mSecondLeft + mTimeBgSize + Utils.dp2px(mContext, 10);
                } else {
                    rectBgRight1 = mSecondLeft + mTimeBgSize ;
                }
                mSecondBgRectF = new RectF(mSecondLeft, topPaddingSize, rectBgRight1, topPaddingSize + mTimeBgSize);
            }

            if (isShowMillisecond && !isShowMilliseconInSecond) {
...
//private boolean isShowMilliseconInSecond = true;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowSecond) {
            ...
            if (isShowMilliseconInSecond) {
                canvas.drawText(Utils.formatNum(mSecond) + "." + Utils.formatMillisecond(mMillisecond), mSecondBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
...
//                }
            ...
            if (isShowMilliseconInSecond) {
                if (!isHideSuffixSecond) {
                    if (mSuffixSecondTextWidth > 0) {
                        // draw second suffix
                        canvas.drawText(mSuffixSecond, Utils.dp2px(mContext,10)+mSecondLeft + mTimeBgSize + mSuffixSecondLeftMargin + (mTimeBgBorderSize * 2), mSuffixSecondTextBaseline, mSuffixTextPaint);
