...
/**
 * Created by ${zipp} on 2018/2/2.
 * 功能描述：viewpager 禁止预加载
 */
...
//            mAdapter.registerDataSetObserver(mObserver);
    ...
    void populate() {
        ...
        if (mPopulatePending) {
            if (DEBUG) LogUtil.i(TAG, "populate is pending, skipping for now...");
            return;
        }
        ...
        if (DEBUG) LogUtil.v(TAG, "populating: startPos=" + startPos + " endPos=" + endPos);
        ...
        for (int i=0; i<mItems.size(); i++) {
            ...
            if ((ii.position < startPos || ii.position > endPos) && !ii.scrolling) {
                if (DEBUG) LogUtil.i(TAG, "removing: " + ii.position + " @ " + i);
                mItems.remove(i);
                i--;
                mAdapter.destroyItem(this, ii.position, ii.object);
                ...
                while (lastPos <= endPos && lastPos < ii.position) {
                    if (DEBUG) LogUtil.i(TAG, "inserting: " + lastPos + " @ " + i);
        ...
        if (lastPos < endPos) {
            ...
            while (lastPos <= endPos) {
                if (DEBUG) LogUtil.i(TAG, "appending: " + lastPos);
    ...
    public void computeScroll() {
        if (DEBUG) LogUtil.i(TAG, "computeScroll: finished=" + mScroller.isFinished());
        if (!mScroller.isFinished()) {
            if (mScroller.computeScrollOffset()) {
                if (DEBUG) LogUtil.i(TAG, "computeScroll: still scrolling");
                int oldX = getScrollX();
                int oldY = getScrollY();
...
//            return super.onInterceptTouchEvent(arg0);
            return superOnIntercepTouchEvent(ev);
        }

    }

    private boolean superOnIntercepTouchEvent(MotionEvent ev) {
        /*
